...
export class AppointmentFiltersComponent implements OnInit {
  ...
  recentFilters = ["12:00 12/14/2020", "12:00 12/14/2020"];
  savedFilters = [
    {
      id: "1",
  ...
  handleSavedFiltersSelect = (filterSelected): void => {
    if (this.selectedSavedFilters.includes(filterSelected.id)) {
      this.selectedSavedFilters = this.selectedSavedFilters.filter(
        (elem) => elem !== filterSelected.id
      );
    } else {
